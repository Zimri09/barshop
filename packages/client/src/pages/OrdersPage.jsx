      ...
      <div className="max-w-4xl mx-auto px-4 mt-8">
        ...
        {authLoading ? (
          ...
          <div className="text-center py-16 bg-slate-900/40 rounded-2xl border border-slate-800 px-6">
            ...
            <p className="text-slate-400 mt-2 text-sm">Create an account or sign in to see your pre-order history.</p>
            <div className="flex flex-wrap justify-center gap-3 mt-6">
              <Link to={`/login?next=${loginNext}`} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-emerald-600 text-white font-bold text-sm">
                <LogIn className="w-4 h-4" /> Sign in
              </Link>
              <Link to={`/login?next=${loginNext}&mode=signup`} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl border border-slate-700 font-bold text-sm">
                <UserPlus className="w-4 h-4" /> Register
              </Link>
            </div>
            <Link to="/customer/browse" className="inline-block mt-4 text-sm text-emerald-400">← Back to shop</Link>
          </div>
        ) : loading ? (
          <div className="flex flex-col items-center justify-center py-20 gap-4">
