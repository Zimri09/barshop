...
// Staff can advance status in a logical flow
const NEXT_STATUS = {
  pending: 'confirmed',
  confirmed: 'ready',
  ready: 'completed',
...
export default function StaffOrders() {
  ...
  async function loadOrders() {
    ...
    try {
      ...
      // Staff: only show preorders (walk-in are already completed)
      setOrders((json.data || []).filter((o) => o.order_type !== 'walk-in' || o.status !== 'completed'))
    } catch (err) {
      console.error(err)
    } finally {
      setLoading(false)
  ...
  return (
    <div className="min-h-screen p-6 bg-gradient-to-br from-slate-900 to-slate-800 text-slate-100">
      <div className="max-w-7xl mx-auto">
        <DashboardHeader title="Order Queue">
                ...
                <div key={order.id} className="rounded-xl bg-white/5 border border-white/10 overflow-hidden">
                  <div className="p-4 flex flex-col sm:flex-row sm:items-center gap-3 justify-between">
                    <div className="flex items-center gap-3 flex-wrap">
                      <button onClick={() => fetchDetails(order.id)} className="text-slate-300 hover:text-white">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {canAdvance && (
                        <button
                          ...
                          className="px-3 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold disabled:opacity-50 capitalize"
                        >
                      ...
                      {canCancel && (
                        <button
                          ...
                          disabled={updatingId === order.id}
                          className="px-3 py-1.5 rounded-lg bg-rose-600/20 border border-rose-500/30 text-rose-400 hover:bg-rose-600/30 text-xs font-bold disabled:opacity-50"
                        >
                          Cancel
                        </button>
                      )}
                      ...
                      <table className="w-full text-sm">
                        ...
                        <tbody className="divide-y divide-slate-800">
                          {(expandedOrder.items || []).map((it) => (
                            <tr key={it.id}>
                              <td className="py-1.5 text-white font-mono text-xs">{it.product_id}</td>
